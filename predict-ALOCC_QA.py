...
preds = np.round(np.squeeze(p_d_of_g))
...
_, y = zip(*(test_flow[i] for i in range(0, len(test_flow), 1)))
y_val = np.vstack(to_categorical(y))[:, 1]
...
for fold in ["fold1", "fold2", "fold3", "fold4", "fold5"]:
    ...
    x_good = [x_val[i] for i, x in enumerate(d_of_g) if x >= -16.8]
    x_poor = [x_val[i] for i, x in enumerate(d_of_g) if x < -16.8 ]

    y_good = [y_val[i] for i, x in enumerate(d_of_g) if x >= -16.8]
    y_poor = [y_val[i] for i, x in enumerate(d_of_g) if x < -16.8 ]
